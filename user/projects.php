...
if (isset($_POST['submit_idea'])) {
    ...
    notifyAdmins($pdo, "Yangi Loyiha G'oyasi! 💡", "$u_name yangi loyiha g'oyasini yubordi: $title", "projects.php");
                ...
                <div class="project-card" style="background: var(--bg-card); border-radius: 16px; overflow: hidden; box-shadow: var(--shadow); border: 1px solid var(--border-color); display: flex; flex-direction: column; transition: 0.3s;" onmouseover="this.style.transform='translateY(-5px)'" onmouseout="this.style.transform='translateY(0)'">
                    <div style="position: relative;">
                        <?php if($project['image_path']): ?>
                            <img src="../<?= $project['image_path'] ?>" style="width: 100%; height: 200px; object-fit: cover;">
                        <?php else: ?>
                            <div style="width: 100%; height: 200px; background: linear-gradient(45deg, #f1f5f9, #e2e8f0); display: flex; align-items: center; justify-content: center; color: #94a3b8; font-size: 3rem;">💡</div>
                        <?php endif; ?>
                        <div style="position: absolute; bottom: 10px; right: 10px; background: rgba(255,255,255,0.9); padding: 5px 12px; border-radius: 20px; font-size: 0.75rem; font-weight: bold; color: var(--primary-color);">
                            <?= htmlspecialchars(strtoupper(isset($project['creator_role']) ? $project['creator_role'] : 'admin')) ?>
                        </div>
                    </div>
                    
                    <div style="padding: 20px; flex: 1; display: flex; flex-direction: column;">
                        <h4 style="margin: 0 0 10px 0; font-size: 1.25rem; color: var(--text-color);"><?= htmlspecialchars($project['title']) ?></h4>
                        <p style="font-size: 0.9rem; color: var(--text-light); margin-bottom: 20px; line-height: 1.6; flex: 1;">
                            <?= nl2br(htmlspecialchars($project['description'])) ?>
                        </p>
                        
                        <div style="display: flex; align-items: center; padding-top: 15px; border-top: 1px solid var(--border-color);">
                            <div style="width: 35px; height: 35px; border-radius: 50%; background: var(--primary-color); color: white; display: flex; align-items: center; justify-content: center; font-weight: bold; margin-right: 12px;">
                                <?= substr(htmlspecialchars(isset($project['creator_name']) ? $project['creator_name'] : 'A'), 0, 1) ?>
                            </div>
                            <div style="font-size: 0.85rem;">
                                <div style="font-weight: bold; color: var(--text-color);"><?= htmlspecialchars(isset($project['creator_name']) ? $project['creator_name'] : 'Admin') ?></div>
                                <div style="color: var(--text-light); font-size: 0.75rem;">Muallif</div>
                            </div>
                        </div>
                    </div>
                </div>
